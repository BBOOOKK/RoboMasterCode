...
class SectorDodgeController:
    def __init__(self):
        ...
        # 圆弧运动参数（可根据需求调整）
        self.radius = 1.0  # 圆弧半径（米）
        self.angular_velocity = 45  # 角速度（度/秒）
        self.linear_velocity = self.calc_linear_velocity()  # 线速度（米/秒）
        
        # 控制参数
        self.control_frequency = 30  # 控制频率（Hz）
    ...
    def sector_dodge(self):
        ...
        print("开始扇形闪避动作...")
        # 1. 向右前方移动（顺时针圆弧）
        self._move_arc(angle=90, clockwise=True)
        time.sleep(0.3)
        # 2. 向左前方移动（逆时针圆弧）
        self._move_arc(angle=180, clockwise=False)
        time.sleep(0.3)
        # 3. 向右前方移动（回到初始方向）
        self._move_arc(angle=90, clockwise=True)
        print("扇形闪避动作完成！")
