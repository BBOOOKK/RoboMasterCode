...
class TLConfig:
    ...
    # 形态学与判定
    kernel_size: int = 5
    area_ratio: float = 0.01   # 面积相对阈值（ROI 的 0.3%）
...
class TrafficLightDetector:
    ...
    def detect(self, bgr: np.ndarray) -> Tuple[str, Dict[str, object]]:
        ...
        green_area, green_c = self._max_area(green_mask)

        color = "none"
        # 红色检测：需要满足面积条件且为圆形（增加宽高比检测）
        if red_area > th and red_area > green_area * self.cfg.dominance:
            if self._is_circular(red_c, self.cfg.min_circularity, self.cfg.min_radius, self.cfg.max_radius, self.cfg.aspect_ratio_max):
                color = "red"
        # 绿色检测：需要满足面积条件且为圆形
        elif green_area > th and green_area > red_area * self.cfg.dominance:
            if self._is_circular(green_c, self.cfg.min_circularity, self.cfg.min_radius, self.cfg.max_radius, self.cfg.aspect_ratio_max):
                 color = "green"
        ...
        red_circularity = 0.0
        red_radius = 0.0
        green_circular = False
        green_circularity = 0.0
        green_radius = 0.0
        ...
        if green_c is not None:
            green_circular = self._is_circular(green_c, self.cfg.min_circularity, self.cfg.min_radius, self.cfg.max_radius)
            area = cv2.contourArea(green_c)
            perimeter = cv2.arcLength(green_c, True)
            if perimeter > 0:
                green_circularity = 4 * np.pi * area / (perimeter * perimeter)
                green_radius = np.sqrt(area / np.pi)
        ...
        info = {
            ...
            "green_circular": green_circular,
            "red_circularity": float(red_circularity),
            "green_circularity": float(green_circularity),           
            "red_radius": float(red_radius),
            "green_radius": float(green_radius)
        }
        return color, info
...
def draw_debug(frame: np.ndarray, info: Dict[str, object], label: str) -> None:
    ...
    elif "green_circular" in info:
        circular_info = f"圆形: {'是' if info['green_circular'] else '否'} 圆形度: {info['green_circularity']:.2f} 半径: {info['green_radius']:.1f}"
        txt = f"{label}  R:{info['red_area']}  G:{info['green_area']}  {circular_info}"
    else:
    ...
    if info.get("red_circular", False) and info["red_cnt"] is not None:
        ...
        radius = int(radius)
        cv2.circle(frame, center, radius, (0, 0, 255), 2)  # 红色圆圈
        cv2.circle(frame, center, 2, (0, 0, 255), 3)       # 红色中心点
    # 如果检测到绿色轮廓且为圆形，绘制绿色圆形
    elif info.get("green_circular", False) and info["green_cnt"] is not None:
        # 计算最小外接圆
        (x, y), radius = cv2.minEnclosingCircle(info["green_cnt"])
        center = (int(x) + x1, int(y) + y1)  # 转换为全局坐标
        radius = int(radius)
        cv2.circle(frame, center, radius, (0, 255, 0), 2)  # 绿色圆圈
        cv2.circle(frame, center, 2, (0, 255, 0), 3)       # 绿色中心点
